...
void pdg::DataDependencyGraph::collectDefUseInfo(llvm::Instruction *inst) {
  // check for def-use dependencies
  for (llvm::Instruction::const_op_iterator cuit = inst->op_begin();
       cuit != inst->op_end(); ++cuit) {
    if (llvm::Instruction * pInst = dyn_cast<llvm::Instruction>(*cuit)) {
...
std::vector<Instruction *> pdg::DataDependencyGraph::getDependencyInFunction(Instruction *pLoadInst) {
  ...
  for (auto StoreInst : StoreVec) {
    MemoryLocation SI_Loc = MemoryLocation::get(StoreInst);
    AliasResult AA_result = aliasAnalysis->alias(LI_Loc, SI_Loc);
    if (AA_result != NoAlias) {
      _flowdep_set.push_back(StoreInst);
    }
  }
  return _flowdep_set;
}

void pdg::DataDependencyGraph::collectCallInstUseInfo(llvm::Instruction *inst) {
...
void pdg::DataDependencyGraph::collectNonLocalMemDependency(llvm::Instruction *inst) {
...
void pdg::DataDependencyGraph::collectDataDependencyInFunc() {
  for (inst_iterator instIt = inst_begin(func), E = inst_end(func); instIt != E; ++instIt) {
    DDG->getNodeByData(instMap[&*instIt]);
    llvm::Instruction *pInstruction = dyn_cast<llvm::Instruction>(&*instIt);
    collectDefUseInfo(pInstruction);
    collectCallInstUseInfo(pInstruction);
    if (isa<LoadInst>(pInstruction)) {
      collectRAWDependency(pInstruction);
      collectNonLocalMemDependency(pInstruction);
    }
  }
}
